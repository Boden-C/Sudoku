...
var grid = [[Cell]]()
for rowNum in  1...gridSize {
    grid.append([])
    for colNum in 1...gridSize {
        grid[rowNum-1].append(Cell(row:rowNum,col:colNum))
    }
}
...
func makeSudokuBoard() {
    ...
    let beginX = (mainWindow.size.width-(cellWidth * 10))/2
    var beginY = (mainWindow.size.height-(cellHeight * 9))/2
    mainWindow.cursor.position = Point(x:beginX, y:beginY)
    
    func drawCell(x:Int, y:Int) {
        let cell = Rect(topLeft:Point(x:x, y:y), size:Size(width:cellWidth+1,height:cellHeight+1))
    ...
    for rowNum in 1...gridSize {
        for colNum in 1...gridSize {
            drawCell(x:currentX, y:currentY)
            mainWindow.cursor.position = Point(x:currentX + (cellWidth/2),y:currentY + (cellHeight/2))
            if grid[rowNum-1][colNum-1].digit != 0 {
                mainWindow.write("\(grid[rowNum-1][colNum-1].digit)")
            } else { mainWindow.write(" ") }
    ...
    for _ in 1...3 {
        edgeLine(x:beginX, y:beginY, edge:"┳")
        middleLine(x:beginX, y:beginY + cellHeight)
        middleLine(x:beginX, y:beginY + (2 * cellHeight))
        edgeLine(x:beginX, y: beginY + (3 * cellHeight), edge:"┻")
        beginY += (3 * cellHeight) + squareHeightGap
    }

    mainWindow.turnOn(redOnBlack)
    drawCell(x:highlightX, y:highlightY)
...
while true {
    ...
    if key.keyType == .isCharacter {
        ...
        case "0":
            msg("Emptied cell (\(cursorPosRow),\(cursorPosCol)).")
            grid[cursorPosRow-1][cursorPosCol-1].digit = 0
            makeSudokuBoard()
        case "1"..."9":
            msg("Entered \(Int(String(key.character!))!) into cell (\(cursorPosRow),\(cursorPosCol)). Press backspace or 0 to remove.")
            grid[cursorPosRow-1][cursorPosCol-1].digit = Int(String(key.character!))!
            makeSudokuBoard()
        case "+":
        ...
        case "-":
            ...
            cellSize -= 1
            makeSudokuBoard()
            msg("Reduced Sudoku Board to Size[\(cellSize)]    Tip: Zoom out with emacs instead")
        case "h":
            showGuide = !showGuide
            if showGuide {
                turnOnGuide()
        ...
        case "r":
            msg("OooOOoo secret. These are the numbers in the following locations: \nSquare:\(grid[cursorPosRow-1][cursorPosCol-1].returnSqr()) \nColumn:\(grid[cursorPosRow-1][cursorPosCol-1].returnCol()) \nRow:\(grid[cursorPosRow-1][cursorPosRow-1].returnRow())")
            makeSudokuBoard()
        case "e":
            screen.shutDown()
            exit(0)
        ...
        switch (key.keyType) {
        case .arrowDown:
            guard cursorPosRow + 1 <= 9 else {
                msg("You can not move down any further")
                continue
            }
            cursorPosRow += 1
            msg("Moved down to cell (\(cursorPosRow),\(cursorPosCol))")
            makeSudokuBoard()
        case .arrowUp: 
